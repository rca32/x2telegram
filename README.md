...
새 Codex 설정은 지능·속도·비용의 균형을 고려한 `GPT-5.6 Terra`와 중간 추론 강도를 사용합니다. 요약은 중립적인 한국어 뉴스 통신사 기사체로 작성되며, 기존에 사람이 선택한 모델 설정은 자동으로 바꾸지 않습니다.
...
> 저장소의 공식 Windows 예약 설치 스크립트로 매일 08:00~22:00 사이에 30분마다 실행되도록 준비해줘. 최근 글은 10개로 제한하고, 실제 예약을 만들기 전에 시간·전송 대상·미리보기·기존 중복 방지 상태를 확인받아.

예약 작업은 로그인된 Windows 사용자 환경에서 콘솔 창 없이 백그라운드로 실행됩니다. PC가 절전 상태면 깨우도록 요청하고, 네트워크가 없거나 실패하면 재시도하도록 설정됩니다. coding agent에게 아래처럼 상태를 물어볼 수 있습니다.

> x2telegram 예약 작업의 다음 실행 시간, 최근 결과, 안전한 실행 로그를 확인해줘. 인증값이나 타임라인 원문은 출력하지 마.

예약 작업의 실제 전송 범위는 미리보기와 같은 최근 10개로 고정됩니다. 최초 자동화를 켜기 전에는 사람이 승인한 수동 전송으로 중복 방지 상태가 한 번 만들어져 있어야 합니다.
